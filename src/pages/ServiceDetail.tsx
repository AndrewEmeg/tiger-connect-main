...
import { ArrowLeft, Star, MessageCircle, Calendar, Heart } from "lucide-react";
...
export default function ServiceDetail() {
    ...
    useEffect(() => {
        const fetchServiceDetails = async () => {
            ...
            try {
                // Fetch the service
                const { data: servicesData, error: serviceError } =
                    await supabase
                        .from("services_table")
                        .select("*")
                        .eq("id", id)
                ...
                if (userError) {
                    console.error("Provider fetch error:", userError);
                }
    ...
    return (
        <AppLayout title={service.title}>
            <div className="container mx-auto px-4 py-8">
                ...
                {/* Reviews Section */}
                <div className="mt-16">
                    <h2 className="text-2xl font-bold mb-6">Service Reviews</h2>
                    <Reviews itemId={id!} itemType="service" />
                </div>
            </div>
        </AppLayout>
    );
